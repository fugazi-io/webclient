...
export class Timer {
	...
	private callback: types.Callback;

	constructor(interval: number, callback: types.Callback) {
	...
	reset(): void {
		this.cancel();
		this.name = window.setTimeout(this.callback, this.interval);
	}
...
export function loadScript(...args: any[]): Promise<HTMLScriptElement> {
	...
	var name: string = types.is(args[1], String) ? args[1] : generateId({prefix: "script:", min: 6, max: 10});
	var cached: boolean = types.is(args[1], Boolean) ? args[1] : (types.is(args[2], Boolean) ? args[2] : true);
...
class ScriptLoader {
	...
	private future: types.Future<HTMLScriptElement>;

	public constructor(url: string, id: string, cached: boolean) {
		this.id = id;
		this.url = url;
		this.cached = cached;
		this.future = new types.Future<HTMLScriptElement>();
	}
	...
	private timedout(): void {
		this.future.reject(new types.Exception("loading of script " + this.id + " timed out"));
	}
}
