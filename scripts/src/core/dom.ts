...
module fugazi.dom {
	...
	export var CREATE_INNER_HTML_KEY: string = "html";
	...
	export function ready(callback: fugazi.Callback): void {
		if (domReady) {
			callback();
		} else {
	...
	export function create(tagName: string, properites?: Object, referenceElement?: any, position: InsertPosition = InsertPosition.Append): Element {
		...
		if (properites) {
			iterator = new collections.PlainObjectIterator<any>(<fugazi.PlainObject<any>> properites);
