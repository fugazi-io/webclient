...
export namespace builder {
	...
	export function create(typeDescriptor: string | descriptor.Descriptor | { [name: string]: any }, parent: componentsBuilder.Builder<components.Component>): componentsBuilder.Builder<Type> {
	...
	abstract class TypeBuilder<T extends Definition> extends componentsBuilder.BaseBuilder<Type, descriptor.Descriptor> {
		...
		protected concreteAssociate(): void {
			...
			if (this.genericsReference || this.genericsBuilder) {
				if (this.genericsBuilder) {
					...
					genericsType = this.resolve(components.ComponentType.Type, this.genericsReference) as Type;
				}
	...
	class StructBuilder extends TypeBuilder<StructDefinition> {
		...
		protected concreteAssociate(): void {
			...
			this.fieldsReferences.keys().forEach(name => {
				fields.set(name, <Type> this.resolve(components.ComponentType.Type, this.fieldsReferences.get(name)));
			});
