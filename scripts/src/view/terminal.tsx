...
export class TerminalView extends view.View<TerminalProperties, TerminalState> {
	private promptResult: commandsHandler.PromptResult;
	...
	public render(): JSX.Element {
		...
		if (this.state.promptMode) {
			const type = this.promptResult.type === "password" ? "password" : "text";
			inputView = <baseInput.PromptInputView
				type={ type }
				message={ this.promptResult.message }
				handler={ this.handlePassword.bind(this) } />
	...
	private onExecute(input: string): void {
		...
		result.then(value => {
			if (commandsHandler.isPromptResult(value)) {
	...
	private handlePassword(password: string) {
		if (password != null) {
			this.promptResult.handler(password);
		}
